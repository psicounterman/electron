(function () {
  ...
  const overrideAPISync = function (module, name, pathArgumentIndex = 0) {
    ...
    module[name] = function () {
      ...
      if (archive === null) throw createError(AsarError.INVALID_ARCHIVE, {asarPath})
  ...
  const overrideAPI = function (module, name, pathArgumentIndex = 0) {
    const old = module[name]
    module[name] = function () {
      const pathArgument = arguments[pathArgumentIndex]
      const {isAsar, asarPath, filePath} = splitPath(pathArgument)
